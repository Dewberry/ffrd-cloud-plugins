...
func GetGeoJSONHandler(ctrl *controller.Controller) echo.HandlerFunc {
	return func(c echo.Context) error {
		var req struct {
			Key        string `json:"key"`
			Projection string `json:"projection"`
		...
		s3Ctrl, err := ctrl.Bh.GetController(req.Bucket)
		if err != nil {
			errMsg := fmt.Errorf("error getting the controller for teh bucket %s: %s", req.Bucket, err.Error())
			log.Error(errMsg.Error())
			return c.JSON(http.StatusInternalServerError, errMsg.Error())
		}
		// Check if the key exists in the S3 bucket
		exists, err := s3Ctrl.KeyExists(req.Bucket, req.Key)
		if err != nil {
			errMsg := fmt.Errorf("Error returned when invoking KeyExists: %s", err.Error())
			log.Error(errMsg.Error())
			return c.JSON(http.StatusInternalServerError, errMsg.Error())
